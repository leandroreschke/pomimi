...
use crate::theme;
use iced::widget::{
    button, center, column, container, row, scrollable, stack, text, text_input, Space,
};
use iced::{time, window, Color, Element, Event, Length, Point, Size, Subscription, Task, Theme};
...
impl PomimiApp {
    pub fn new() -> (Self, Task<Message>) {
        ...
        let fonts = Task::batch(vec![
            iced::font::load(std::borrow::Cow::Borrowed(
                include_bytes!("../assets/fonts/SpaceGrotesk-Regular.ttf").as_slice(),
            ))
            .map(Message::FontLoaded),
            iced::font::load(std::borrow::Cow::Borrowed(
                include_bytes!("../assets/fonts/SpaceGrotesk-Bold.ttf").as_slice(),
            ))
            .map(Message::FontLoaded),
            iced::font::load(std::borrow::Cow::Borrowed(
        ...
        let connect_db = Task::perform(
            async { Database::new().await.map_err(|e| e.to_string()) },
            Message::DbConnected,
        );

        (PomimiApp::Loading, Task::batch(vec![fonts, connect_db]))
    }
    ...
    pub fn update(&mut self, message: Message) -> Task<Message> {
        match self {
            PomimiApp::Loading => match message {
                Message::DbConnected(Ok(db)) => {
                    ...
                    let load_color = Task::perform(
                        {
                            let db = db.clone();
                            async move { db.get_accent_color().await.map_err(|e| e.to_string()) }
                        },
                        Message::ColorLoaded,
                    );
                    ...
                    let load_theme = Task::perform(
                        {
                            let db = db.clone();
                            async move { db.get_theme_preference().await.map_err(|e| e.to_string()) }
                        },
                        Message::ThemeLoaded,
                    );
            ...
            PomimiApp::Loaded(state) => {
                match message {
                    ...
                    Message::ColorLoaded(Ok(None)) => Task::none(),
                    ...
                    // UI
                    Message::ToggleMiniMode => match state.view_mode {
                        ViewMode::Full => {
                            ...
                            window::latest().and_then(move |id| {
                                ...
                                if current_pos != Point::ORIGIN {
                                    let new_x =
                                        current_pos.x + current_size.width - mini_size.width;
                        ...
                        ViewMode::Mini => {
                            ...
                            window::latest().and_then(move |id| {
                                ...
                                if current_pos != Point::ORIGIN {
                                    let new_x =
                                        current_pos.x + current_size.width - full_size.width;
                                    let new_pos = Point::new(new_x, current_pos.y);
                                    tasks.push(window::move_to(id, new_pos));
                                }
                    ...
                    Message::SetColor(color) => {
                        ...
                        Task::perform(
                            async move {
                                db.save_accent_color(color.r, color.g, color.b)
                                    .await
                                    .map_err(|e| e.to_string())
                            },
                            |result| {
                    ...
                    Message::SetRequireConfirmation(val) => {
                        ...
                        Task::perform(
                            async move {
                                db.save_require_confirmation(val)
                                    .await
                                    .map_err(|e| e.to_string())
                            },
                            |result| {
                    ...
                    Message::ToggleTheme => {
                        ...
                        Task::perform(
                            async move { db.save_theme(is_dark).await.map_err(|e| e.to_string()) },
                            |result| {
                                if let Err(e) = result {
            ...
            PomimiApp::Loaded(state) => {
                ...
                let content: Element<Message> = if state.view_mode == ViewMode::Mini {
                    let active_task_view: Element<'_, Message> = match state.timer.phase {
                        Phase::ShortBreak | Phase::LongBreak => {
                            container(text("REST").size(12).font(iced::Font {
                                weight: iced::font::Weight::Bold,
                                ..iced::Font::DEFAULT
                            }))
                            .padding(10)
                        ...
                        Phase::Focus => {
                            if let Some(id) = state.active_task_id {
                                if let Some(task) = state.tasks.iter().find(|t| t.id == id) {
                                    container(
                                        row![
                                            container(Space::new().width(6).height(6)).style(
                                                |_t: &Theme| container::Style {
                                                    background: Some(state.primary_color.into()),
                                                    ..container::Style::default()
                                                }
                                            ),
                                            column![text(&task.text).size(12).font(iced::Font {
                                                weight: iced::font::Weight::Bold,
                                                ..iced::Font::DEFAULT
                                            }),]
                                        ]
                    ...
                    let main_content =
                        column![timer_view, Space::new().height(20), tasks_view, footer]
                            .padding(40)
                            .width(Length::Fill)
                            .height(Length::Fill)
                            .max_width(500)
                ...
                if state.active_modal != Modal::None {
                    let modal_content = match state.active_modal {
                        Modal::AddTask => column![
                            text("Add New Task").size(18).font(iced::Font {
                                weight: iced::font::Weight::Bold,
                                ..iced::Font::DEFAULT
                            }),
                        ...
                        Modal::Settings => {
                            let color_btn = |color: Color| {
                                button(container(Space::new().width(20).height(20)).style(
                            ...
                            column![
                                text("Settings").size(18).font(iced::Font {
                                    weight: iced::font::Weight::Bold,
                                    ..iced::Font::DEFAULT
                                }),
                                ...
                                row![
                                    components::checkbox::checkbox(
                                        ...
                                        state.primary_color,
                                        Message::SetRequireConfirmation(
                                            !state.require_confirmation
                                        )
                                    ),
                        ...
                        Modal::TimerSettings => column![
                            text("Timer Settings").size(18).font(iced::Font {
                                weight: iced::font::Weight::Bold,
                                ..iced::Font::DEFAULT
                            }),
                            ...
                            row![
                                button(text("25/5"))
                                    .on_press(Message::SetDuration(25 * 60))
                                    .style(components::button::secondary)
                                    .width(Length::Fill),
                                button(text("50/10"))
                                    .on_press(Message::SetDuration(50 * 60))
                                    .style(components::button::secondary)
                                    .width(Length::Fill),
                            ]
                            .spacing(10),
    ...
    fn view_tasks<'a>(&self, state: &'a State) -> Element<'a, Message> {
        if state.tasks.is_empty() {
            return container(
                row![
                    text("All tasks done").size(14).color(theme::TEXT_DIM),
                    button(text("Add task").size(14))
                        .on_press(Message::OpenModal(Modal::AddTask))
                        .style(components::button::secondary)
                        .padding([8, 12])
                ]
                .spacing(12)
        ...
        let items: Element<'a, Message> = scrollable(
            column(state.tasks.iter().map(|task| {
                if state.pending_completion_task_id == Some(task.id) {
                    ...
                    row![
                        components::checkbox::checkbox(
                            is_active,
                            state.primary_color,
                            Message::SetActiveTask(task.id)
                        ),
                        container(
                            column![text(&task.text)
                                .size(14)
                                .font(iced::Font {
                                    weight: iced::font::Weight::Bold,
                                    ..iced::Font::DEFAULT
        ...
        column![
            ...
            container(Space::new().height(1))
                .style(|_t: &Theme| container::Style {
                    background: Some(theme::TEXT_DIM.into()),
                    ..container::Style::default()
                })
                .width(Length::Fill),
            items,
